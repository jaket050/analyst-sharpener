...
const DOMAIN_PACKS = {
  ...
  marketing: {
    ...
    icon: "📣",
    color: "#c084fc",
    description: "Acquisition metrics, attribution, LTV and CAC, funnel analysis, email analytics, segmentation, incrementality, campaign measurement",
    questions: [
      // Basic
      { level: "Basic", q: "What is CAC (Customer Acquisition Cost) and how is it calculated?", a: "CAC = Total sales and marketing spend / Number of new customers acquired in the same period. Example: $100,000 spend that acquired 500 customers = $200 CAC. Best practice: segment by channel — paid search CAC, organic CAC, paid social CAC — because blended CAC hides channel-level inefficiency. Always compare to LTV: a healthy business targets LTV:CAC ≥ 3:1. If LTV:CAC < 1, you lose money on every customer. Calculate on a cohort basis (spend in period X / customers acquired in period X) rather than mixing spend and acquisitions from different periods — mixing produces a meaningless blended number." },
      { level: "Basic", q: "What is LTV (Customer Lifetime Value) and how do you calculate it?", a: "LTV estimates total revenue a customer generates over their entire relationship with the business. Simple formula: LTV = AOV × Purchase Frequency × Customer Lifespan. More accurate: LTV = Gross Margin per Order × Purchase Frequency × Lifespan — gross margin LTV determines whether acquisition is actually profitable. Predictive LTV uses statistical models (BG/NBD for non-subscription businesses) that estimate future purchasing from behavioral history. LTV is used to: justify CAC spend, set acquisition bid caps, segment customers by value, and prioritize retention investment. The most common mistake is using revenue LTV instead of gross margin LTV — a $200 CAC looks fine against $400 revenue LTV until you account for 40% COGS." },
      { level: "Basic", q: "What is ROAS (Return on Ad Spend) and how is it different from ROI?", a: "ROAS = Revenue attributed to ads / Ad spend. A 5× ROAS means $5 in revenue per $1 spent. ROAS is NOT ROI — it ignores COGS, fulfillment, and overhead. A 5× ROAS looks healthy but if gross margin is 20%, you are losing money on every sale. ROI = (Revenue − All Costs) / All Costs — the true profitability measure. Set target ROAS by working backward from required margin: if gross margin is 40% and you need 10% net margin, minimum ROAS = 1 / (40% − 10%) ≈ 3.3×. Use ROAS for comparing campaigns within a channel; use ROI or contribution margin for evaluating whether a channel is worth running at all." },
      { level: "Basic", q: "What is CTR (Click-Through Rate) and what does a low CTR indicate?", a: "CTR = Clicks / Impressions × 100. A 1% CTR means 1 in 100 people who saw the ad clicked it. Low CTR signals that the creative, subject line, or headline is not compelling enough to prompt action. However, CTR alone is misleading: high CTR + low conversion rate means you are attracting the wrong audience. Low CTR + high conversion rate means the creative is selective but effective. Benchmark CTR varies by channel: Google Search 2-5%, Display 0.1%, Facebook 0.9%, email 2-3%. Always pair CTR with CVR and CPA for a complete picture. Optimizing CTR in isolation can maximize irrelevant clicks that worsen CPA and ROAS." },
      { level: "Basic", q: "What is a marketing conversion funnel and how do you measure drop-off at each stage?", a: "A conversion funnel tracks users from awareness to purchase through sequential stages: Impression → Click → Landing Page → Add to Cart → Purchase. Each stage has a conversion rate = users who completed the stage / users who entered it. SQL approach: COUNT DISTINCT user_ids at each funnel event ordered by timestamp, then compute step-over-step ratios. A 70% Cart-to-Purchase drop-off signals checkout friction. High Click-to-Landing drop-off suggests page load speed issues or mismatch between ad promise and page content. The funnel tells you WHERE to intervene — without it, optimization is guesswork. Segment by device and channel; a 3% mobile conversion rate vs 7% desktop conversion rate is a mobile UX problem, not a traffic problem." },
      { level: "Basic", q: "What is marketing attribution and why is it difficult to get right?", a: "Attribution assigns credit for a conversion to one or more marketing touchpoints in the customer journey. It is difficult because: (1) Multi-touch journeys — a customer sees a Facebook ad, clicks a Google search ad, opens an email, then converts. Which gets credit? (2) Walled gardens — Meta, Google, and Apple each report their own attribution numbers using their own logic, so summing across platforms double-counts conversions. (3) Attribution window matters — 1-day vs. 7-day click window gives completely different results for the same campaign. (4) Offline-to-online gap — TV drives brand search that drives conversions, but TV gets no credit in digital attribution. No model is correct. The goal is to choose a model that best informs the specific budget decision you are trying to make." },
      { level: "Basic", q: "What is CPL (Cost Per Lead) and when would you use it instead of CAC?", a: "CPL = Total spend / Number of leads generated. CPL is used when the sales cycle is long enough that you cannot quickly calculate CAC — common in B2B, healthcare, and financial services where months pass between lead capture and deal close. A low CPL is meaningless without Lead-to-Customer Conversion Rate — cheap leads that never close are waste. Effective CAC = CPL / Lead-to-Customer Rate. SQL: SELECT campaign, SUM(spend) / COUNT(DISTINCT lead_id) AS cpl, COUNT(DISTINCT CASE WHEN converted = 1 THEN lead_id END) * 1.0 / COUNT(DISTINCT lead_id) AS lead_cvr FROM campaigns c JOIN leads l ON c.campaign_id = l.source_campaign GROUP BY campaign ORDER BY cpl;" },
      { level: "Basic", q: "Write SQL to calculate conversion rate, revenue, and ROAS by acquisition channel.", a: "SELECT acquisition_channel,\n  COUNT(DISTINCT session_id) AS sessions,\n  COUNT(DISTINCT CASE WHEN converted = 1 THEN session_id END) AS conversions,\n  ROUND(COUNT(DISTINCT CASE WHEN converted = 1 THEN session_id END) * 100.0\n        / COUNT(DISTINCT session_id), 2) AS conversion_rate_pct,\n  SUM(CASE WHEN converted = 1 THEN order_value ELSE 0 END) AS total_revenue,\n  SUM(CASE WHEN converted = 1 THEN order_value ELSE 0 END)\n        / NULLIF(SUM(spend), 0) AS roas\nFROM sessions s\nLEFT JOIN channel_spend cs\n  ON s.acquisition_channel = cs.channel\n  AND DATE_TRUNC('month', s.session_date) = cs.spend_month\nGROUP BY acquisition_channel\nORDER BY total_revenue DESC;\n\nNULLIF prevents division by zero for organic channels with no spend. DATE_TRUNC aligns spend period to session period — missing this step produces inflated or deflated ROAS by mixing time windows." },
      { level: "Basic", q: "What is cohort analysis in marketing and why is it more useful than aggregate metrics?", a: "A cohort groups customers by acquisition period — typically month of first purchase. Cohort analysis tracks each group's behavior over time: repeat purchase rate, LTV, and churn rate at months 1, 3, 6, and 12 post-acquisition. Why it beats aggregates: (1) Aggregate revenue going up could mean a large low-LTV cohort or a small high-LTV cohort — very different business implications that aggregates hide. (2) Cohort curves reveal whether product or marketing changes are improving retention — newer cohorts with flatter churn curves signal real improvement. (3) LTV measured at the cohort level is the correct method — blending customers acquired at different times with different behaviors produces a meaningless average. Cohort analysis is the diagnostic layer between 'revenue went up' and understanding why." },
      { level: "Basic", q: "What is the difference between a session and a user in web analytics, and why does it matter for reporting?", a: "A user is a unique individual identified by cookie, device ID, or login. A session is one continuous visit with no more than 30 minutes of inactivity — one user generates many sessions. Why it matters: (1) Conversion rate per session (what % of visits convert) vs. per user (what % of visitors ever convert) answer different questions and produce different numbers. (2) Session-based attribution can credit the same user multiple times across sessions, inflating channel performance. (3) GA4 shifted to user-centric and event-based measurement — knowing user vs. session vs. event is essential for reading reports correctly. (4) Multi-device journeys fragment one user across multiple sessions and cookies, making true user-level analysis require login data or probabilistic identity resolution." },
      // Intermediate
      { level: "Intermediate", q: "Explain first-touch, last-touch, and linear attribution models and when to use each.", a: "First-touch: 100% credit to the first channel the customer touched. Use when measuring awareness and top-of-funnel efficiency. Overvalues brand awareness channels, ignores channels that close conversions. Last-touch: 100% credit to the channel immediately before conversion. Use when bottom-of-funnel conversion efficiency is the priority. Overvalues retargeting and branded search that harvest existing intent rather than creating it. Linear: equal credit to every touchpoint. Use as a neutral baseline — least wrong for complex journeys but dilutes credit across all channels. Time decay: more credit to touchpoints closer to conversion — suited for long sales cycles. Data-driven: ML assigns credit based on actual conversion probability contribution at each step — best option but requires high data volume and is opaque. Choose model based on the decision you are making, not which channel looks best under it." },
      { level: "Intermediate", q: "Write SQL to calculate 12-month LTV by acquisition cohort from a historical orders table.", a: "WITH customer_base AS (\n  SELECT customer_id,\n    DATE_FORMAT(MIN(order_date), '%Y-%m') AS acquisition_cohort,\n    MIN(order_date) AS first_order_date\n  FROM orders\n  GROUP BY customer_id\n),\npost_acquisition AS (\n  SELECT cb.customer_id, cb.acquisition_cohort,\n    SUM(o.order_value) AS ltv_12m,\n    SUM(o.gross_margin) AS margin_12m,\n    COUNT(DISTINCT o.order_id) AS total_orders\n  FROM customer_base cb\n  JOIN orders o\n    ON cb.customer_id = o.customer_id\n    AND o.order_date <= DATE_ADD(cb.first_order_date, INTERVAL 365 DAY)\n  GROUP BY cb.customer_id, cb.acquisition_cohort\n)\nSELECT acquisition_cohort,\n  COUNT(DISTINCT customer_id) AS cohort_size,\n  ROUND(AVG(ltv_12m), 2) AS avg_ltv_12m,\n  ROUND(AVG(margin_12m), 2) AS avg_margin_12m,\n  ROUND(AVG(total_orders), 2) AS avg_orders\nFROM post_acquisition\nGROUP BY acquisition_cohort\nORDER BY acquisition_cohort;\n\nThis shows whether customer quality is improving or degrading across acquisition cohorts — the most important trend a DTC analyst can track." },
      { level: "Intermediate", q: "What is RFM segmentation and how would you implement it in SQL?", a: "RFM segments customers by Recency (days since last purchase), Frequency (purchase count), and Monetary value (total spend) to target marketing with the right message to the right segment. SQL:\nWITH rfm_raw AS (\n  SELECT customer_id,\n    DATEDIFF(CURRENT_DATE, MAX(order_date)) AS recency_days,\n    COUNT(DISTINCT order_id) AS frequency,\n    SUM(order_value) AS monetary\n  FROM orders GROUP BY customer_id\n),\nscored AS (\n  SELECT *,\n    NTILE(5) OVER (ORDER BY recency_days ASC) AS r,\n    NTILE(5) OVER (ORDER BY frequency DESC) AS f,\n    NTILE(5) OVER (ORDER BY monetary DESC) AS m\n  FROM rfm_raw\n)\nSELECT *, r+f+m AS total_score,\n  CASE\n    WHEN r >= 4 AND f >= 4 THEN 'Champions'\n    WHEN r >= 3 AND f >= 3 THEN 'Loyal'\n    WHEN r >= 4 AND f <= 2 THEN 'New Customers'\n    WHEN r <= 2 AND f >= 3 THEN 'At Risk'\n    ELSE 'Needs Attention'\n  END AS segment\nFROM scored;\n\nChampions get loyalty rewards. At Risk get win-back campaigns. New Customers get onboarding sequences. Segment-specific messaging outperforms mass campaigns on both conversion rate and unsubscribe rate." },
      { level: "Intermediate", q: "What is email deliverability and what metrics signal problems?", a: "Deliverability is whether your emails reach the inbox vs. spam folder or bounce entirely. Key metrics: (1) Delivery rate = delivered / sent. Below 95% signals list hygiene issues. (2) Open rate — below 15% general or 20% e-commerce signals subject line problems or damaged sender reputation. (3) Spam complaint rate — the critical threshold. Above 0.1% (1 complaint per 1,000 emails) causes Gmail and Outlook to route all your mail to spam. Fix this before anything else. (4) Hard bounce rate — invalid addresses. Above 2% means clean the list immediately. (5) Unsubscribe rate — above 0.5% per send suggests audience-content mismatch or sending too frequently. Problems compound: high complaints damage sender reputation, which lowers delivery rates, which lowers open rates, which creates misleading engagement data. Address complaint rate first, then bounce hygiene, then content optimization." },
      { level: "Intermediate", q: "What is the difference between marketing-sourced and marketing-influenced pipeline in B2B?", a: "Marketing-sourced pipeline: deals where marketing created the first contact — inbound form, content download, paid lead. Marketing owns origination. Marketing-influenced pipeline: any deal where marketing had any touchpoint before close, even if sales initiated first contact. Influenced is always larger than sourced. Why it matters: (1) Sourced pipeline is the metric CMOs are held accountable to. (2) Influenced demonstrates marketing's broader revenue contribution beyond lead generation. (3) Attribution disputes happen at the boundary — sales claims influenced deals as sourced, marketing overclaims influence on deals they barely touched. SQL approach: JOIN CRM opportunities to marketing touchpoints (events, email clicks, content downloads) within a defined window before opportunity creation (e.g., 90 days). Flag sourced (first CRM activity = marketing-owned channel) vs. influenced (any marketing touchpoint in window)." },
      { level: "Intermediate", q: "Write Python to analyze campaign performance across segments and flag underperformers.", a: "import pandas as pd\nimport numpy as np\n\n# df columns: campaign_id, segment, impressions, clicks, conversions, spend, revenue\ndf['ctr'] = df['clicks'] / df['impressions']\ndf['cvr'] = df['conversions'] / df['clicks'].replace(0, np.nan)\ndf['cpa'] = df['spend'] / df['conversions'].replace(0, np.nan)\ndf['roas'] = df['revenue'] / df['spend'].replace(0, np.nan)\n\n# Benchmarks = 75th percentile across all segments\nbenchmarks = {\n    'ctr': df['ctr'].quantile(0.75),\n    'cvr': df['cvr'].quantile(0.75),\n    'roas': df['roas'].quantile(0.75),\n}\n\ndf['flag_ctr'] = df['ctr'] < benchmarks['ctr']\ndf['flag_cvr'] = df['cvr'] < benchmarks['cvr']\ndf['flag_roas'] = df['roas'] < benchmarks['roas']\ndf['flags'] = df[['flag_ctr','flag_cvr','flag_roas']].sum(axis=1)\n\n# Underperformers on 2+ metrics, sorted by spend (biggest waste first)\nflagged = df[df['flags'] >= 2].sort_values('spend', ascending=False)\nprint(flagged[['campaign_id','segment','spend','ctr','cvr','roas','flags']])" },
      { level: "Intermediate", q: "What is incrementality testing and why does standard attribution overstate channel value?", a: "Incrementality measures whether a channel or campaign CAUSED additional conversions vs. conversions that would have happened organically. Standard attribution overstates value because it claims credit for organic demand — a retargeting ad that fires at a customer who was already going to buy. Test setup: randomly withhold ads from a holdout group (ghost ads or geo holdout), compare conversion rate between exposed and holdout groups. Incremental lift = (CVR exposed − CVR holdout) / CVR holdout. True incremental ROAS = incremental revenue / spend. A retargeting campaign reporting 8× ROAS may have only 2× incremental ROAS — the other 6× was organic demand the ad took credit for. Without incrementality, marketers systematically over-invest in retargeting and branded search that harvest existing demand and under-invest in prospecting that creates new demand." },
      { level: "Intermediate", q: "How do you calculate share of voice (SOV) and why does it predict market share growth?", a: "SOV = Your brand impressions / Total category impressions × 100. In paid media: your impressions / (your impressions + all competitor impressions) in the category. In earned media: your brand mentions / total category mentions. Research supports the SOV-to-market-share relationship: brands with SOV above their market share tend to grow (excess SOV = latent demand creation); brands with SOV below their market share tend to decline. If your brand has 20% market share but only 10% SOV, you are likely losing ground and need to increase media weight to defend position. SQL for paid auction data: SELECT SUM(CASE WHEN brand = 'OurBrand' THEN impressions ELSE 0 END) * 100.0 / SUM(impressions) AS sov_pct FROM auction_impressions WHERE category = 'target_category'. Track monthly vs. market share with 6-month lag to validate the predictive relationship in your category." },
      { level: "Intermediate", q: "How would you design an A/B test for a marketing email subject line? Walk through the full setup.", a: "(1) Hypothesis: Subject line B (personalized with first name and product category) will increase open rate by at least 2pp vs. control. (2) Primary metric: Open rate. Guardrail: unsubscribe rate must not increase significantly. (3) Sample size: baseline open rate 22%, MDE = 2pp absolute, alpha = 0.05, power = 0.80 requires approximately 4,000 subscribers per arm (8,000 total). (4) Randomization: 50/50 random split at send time, stratified by engagement tier (active vs. at-risk) to ensure balanced groups. (5) Send simultaneously — never split by time of day. Morning vs. afternoon open rates differ systematically and contaminate results. (6) Measure opens for 48 hours post-send (captures 90%+ of total opens for most audiences). (7) Analysis: chi-square test for difference in proportions. p < 0.05 and lift > 2pp: deploy B. Also check: did unsubscribe rate increase? (8) Repeat 2-3 times before declaring a winner. Single-send results have high variance; confirming across multiple sends is the correct standard for email tests." },
      // Advanced
      { level: "Advanced", q: "Design a full marketing analytics framework for a DTC e-commerce brand at $5M ARR.", a: "Build in tiers. Tier 1 (foundation — build first): Acquisition dashboard — CAC by channel, new customers per week, channel spend mix vs. prior year. Revenue dashboard — MoM revenue, AOV, new vs. returning customer revenue split. These answer what leadership asks every week. Without clean Tier 1, no higher layer is credible. Tier 2 (growth): Cohort LTV analysis — 3/6/12-month LTV by acquisition cohort and by channel. Are we acquiring better or worse customers over time? Multi-touch attribution — contribution across paid social, search, email, organic. Funnel analysis — step-conversion rates by device and channel. Tier 3 (optimization): RFM segmentation for targeted campaigns. Churn prediction for at-risk customers. Incrementality testing to validate channel ROI. Data stack: GA4 + Shopify → BigQuery → dbt → Looker or Tableau. The mistake at this stage: building Tier 3 predictive models before Tier 1 is reliable. Fix CAC tracking and cohort analysis first. A churn model nobody trusts is worse than no model." },
      { level: "Advanced", q: "What is marketing mix modeling (MMM) and how does it compare to multi-touch attribution?", a: "MMM uses regression on aggregate data — weekly revenue vs. weekly spend by channel plus seasonality, price, and macroeconomic factors — to estimate each channel's revenue contribution without user-level tracking. Multi-touch attribution uses individual user journeys (cookies, click logs, device graphs) to assign credit to specific touchpoints. Key differences: MMM works in a cookieless world and captures offline channels (TV, radio, OOH) but requires 2-3 years of historical data and updates slowly — better for quarterly budget decisions. MTA is granular and fast but is degrading as third-party cookies disappear and walled gardens cannot be cross-platform measured. Best practice: use MMM for strategic budget allocation (which channels get funded) and in-platform last-touch or data-driven attribution for tactical optimization (which creatives and audiences to scale). Privacy changes from iOS 14.5 and cookie deprecation have accelerated MMM adoption as MTA becomes unreliable above the channel level." },
      { level: "Advanced", q: "How do you measure ROI of a brand awareness campaign that generates no direct conversions?", a: "Brand campaigns build future intent, not immediate conversions. Measurement approaches: (1) Brand lift study: survey exposed vs. unexposed audiences on unaided awareness, ad recall, and purchase intent. Meta and YouTube offer built-in lift studies for large campaigns. (2) Search volume lift: compare branded keyword search volume in exposed vs. unexposed geographies during and after the campaign. Proxy for intent creation. (3) Geo experiment with difference-in-differences: run campaign in select DMAs, withhold in others, compare revenue per user controlling for pre-period baseline differences. (4) Share of voice change: did SOV increase vs. competitors? SOV growth predicts market share growth 6-12 months out. (5) Marketing mix model: MMM can attribute TV and display investment to total revenue including lagged and halo effects on paid search and direct. The mistake: applying direct ROAS targets to brand campaigns. A brand campaign with 0.5× direct ROAS that lifts conversion rates 15% across all downstream channels may be the highest ROI investment in the portfolio — but last-click attribution will never show it." },
      { level: "Advanced", q: "GA4 shows one channel drives 40% of your conversions. Walk through your investigation before acting on it.", a: "Before acting: (1) Attribution model — what model is GA4 using? Default is data-driven. Switch to first-touch and last-touch and compare. If the 40% share collapses under first-touch, this channel is a bottom-funnel harvester claiming credit for demand created elsewhere. (2) Attribution window — does a 30-day lookback inflate credit? Shorten to 7 days and compare the shift. (3) Direct traffic quality — Direct absorbs dark social, email clicks without UTMs, mobile app opens, and attribution failures. Split Direct by landing page to see what it actually contains. (4) Self-reported attribution — add 'How did you hear about us?' at checkout. Self-reported often tells a completely different story than modeled attribution. (5) Incrementality — run a geo holdout for that channel. Is the 40% contribution causal or correlational? (6) Seasonality — is this elevated share recent? Plot 12-month channel mix trend. A channel gaining share may be benefiting from a brand campaign measured elsewhere. (7) Cross-device gaps — GA4 underrepresents mobile-heavy journeys without user login. The channel claiming credit may not be where discovery happened." },
      { level: "Advanced", q: "Walk through how you would build a customer churn prediction model for a subscription e-commerce brand.", a: "(1) Define churn: for subscription, churn = cancellation event. For non-subscription DTC, churn = no purchase in N days where N = 2× average repurchase interval for the category. (2) Feature engineering: recency (days since last order), frequency (orders in last 90 and 180 days), monetary (spend last 12 months), email engagement (open and click rate last 90 days), site visits last 30 days, support contacts (high support volume is a strong churn signal), tenure (months as customer), product category (some SKUs have structural churn rates). (3) Model: XGBoost or LightGBM — handles non-linear relationships, missing data, and class imbalance. Train on customers who existed 6 months ago, target whether they churned in the next 90 days. (4) Evaluation: AUROC > 0.75 is solid. Check precision at top decile — flagged customers should have high actual churn rates so intervention spend is targeted. (5) Operationalize: score all active customers weekly. Top 20% flagged triggers reactivation email sequence, discount for mid-LTV accounts, service outreach for high-LTV. (6) Measure: track churn rate for flagged-and-intervened vs. flagged-holdout monthly. Revenue saved minus intervention cost = program ROI." },
      { level: "Advanced", q: "You are the first marketing analyst at a 50-person startup with no analytics infrastructure. How do you prioritize what to build?", a: "Week 1-2: audit existing data. What is in GA4, the CRM, Shopify or billing? Map what data exists before building anything. Do not start building dashboards before you understand the inputs. Week 2-4: define ground truth metrics. CAC, LTV, and new customer growth will be contested by multiple stakeholders — align on definitions before data exists. These definitions govern every downstream decision. Month 2: build the acquisition dashboard first. CAC by channel, new customers per week, channel spend. This answers what the CEO and CMO ask every Monday. Establish credibility with the basics. Month 3: cohort retention analysis. Are acquired customers coming back? 90-day LTV by channel? This determines where acquisition investment should actually go. Month 4: attribution model. Only after you understand acquisition quality and retention do you know what attribution question to answer. Month 6 onward: RFM segmentation, churn prediction, incrementality testing. The mistake: building predictive models before ground truth metrics are trusted. Every sophisticated model will be dismissed if stakeholders don't trust the underlying numbers. Credibility is the foundation. Build reporting first, then analysis, then modeling." },
    ]
  }
};
